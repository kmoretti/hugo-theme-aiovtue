...
/**
 * Parse the simple friend-link YAML shape used by hexo-blog-source.
 * It intentionally accepts only the documented list/group/link structure,
 * rather than pretending to be a complete YAML parser.
 */
  ...
  for (const rawLine of String(text || '').replace(/^\uFEFF/, '').split(/\r?\n/)) {
    ...
    if (indent === 2) {
      ...
      if (pair?.key === 'link_list') {
        inLinkList = true
        inTags = false
      } else if (pair && Object.hasOwn(GROUP_FIELD_MAP, pair.key)) {
        group[GROUP_FIELD_MAP[pair.key]] = pair.value
        inTags = false
      }
      continue
    }
    ...
    if (indent === 6) {
      ...
      if (pair?.key === 'tags' && !pair.value) {
        inTags = true
        link.tags = []
      } else if (pair && Object.hasOwn(LINK_FIELD_MAP, pair.key)) {
        link[LINK_FIELD_MAP[pair.key]] = pair.value
        inTags = false
      } else {
        inTags = false
      }
